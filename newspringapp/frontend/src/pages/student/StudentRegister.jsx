...
const StudentRegister = () => {
  ...
  const handleSendOTP = async () => {
    if (!isValidEmail(email)) return toast.warn("Enter a valid email before requesting OTP.");
    try {
      setLoading(true);
  ...
  const handleVerifyOtp = async () => {
    if (!email || !otp) return toast.warn("Email and OTP required to verify.");
    try {
      setLoading(true);
      const response = await verifyStudentOtp({ email: email.trim(), otp: otp.trim() });
  ...
  const handleRegister = async (e) => {
    ...
    if (!isValidEmail(email)) return toast.warn("Invalid email format.");
    if (!batch.trim() || !department.trim()) return toast.warn("Batch and Department are required.");
    if (!isOtpVerified) return toast.warn("Verify OTP before registering.");
    ...
    try {
      setLoading(true);
      const emailExists = await checkStudentEmailExists(email.trim());
      if (emailExists) return toast.warn("This email is already registered.");
              ...
              <form onSubmit={handleRegister} className="space-y-5">
                ...
                <div className="flex gap-3 items-center">
                  <input
                    ...
                    className={`input-style flex-1 ${!otpSent ? "bg-gray-100 cursor-not-allowed" : ""}`}
                    value={otp}
